...
export default function UserProfileModal({
  ...
  const toggleFollow = () => {
    ...
    if (isFollowing) {
      updatedFollowing = currentFollowing.filter((id) => id !== profileUser.id);
      setIsFollowing(false);
    } else {
      updatedFollowing = [...currentFollowing, profileUser.id];
      setIsFollowing(true);
    }

    AuthService.updateProfile(currentUser.id, { following: updatedFollowing });
  };
      ...
      <div className="relative w-full max-w-xl border border-white/15 bg-[#0a0a0a] shadow-2xl p-6 sm:p-8 max-h-[92vh] overflow-y-auto">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-4 text-[10px] text-neutral-500 mt-2">
              <span className="flex items-center gap-1">
                ...
                Katılım: {new Date(profileUser.createdAt).toLocaleDateString("tr-TR")}
              </span>
            </div>
          </div>
        </div>

        {/* Actions: Follow / DM (If not own profile) */}
        {!isMe && currentUser && (
          <div className="flex gap-3 mb-6">
            <Button
              onClick={toggleFollow}
              size="sm"
              className={`flex-1 font-bold text-xs uppercase rounded-none py-2.5 ${
        ...
        <div className="space-y-3 mb-6">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block flex items-center gap-1.5">
            <ListMusic className="h-3.5 w-3.5 text-red-500" /> OLUŞTURDUĞU ÇALMA LİSTELERİ ({userPlaylists.length})
          </span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
              {userPlaylists.map((pl) => (
                <div key={pl.id} className="p-2.5 border border-white/10 bg-black/60 flex items-center gap-2.5">
                  <img src={pl.coverImage} alt={pl.title} className="h-10 w-10 object-cover border border-white/20 shrink-0" />
                  <div className="min-w-0">
                    <h4 className="text-xs font-bold text-white truncate">{pl.title}</h4>
                    <p className="text-[9px] text-neutral-400">{pl.trackIds.length} Parça</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="space-y-3">
          <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block flex items-center gap-1.5">
            <Disc3 className="h-3.5 w-3.5 text-amber-500" /> PAYLAŞTIĞI MİXLER ({userMixes.length})
          </span>
          ...
          {userMixes.length === 0 ? (
            ...
            <div className="space-y-2">
              {userMixes.map((m) => (
                <div key={m.id} className="p-2.5 border border-white/10 bg-black/60 flex items-center justify-between gap-3">
...
with open("src/components/UserProfileModal.tsx", "w", encoding="utf-8") as f:
    f.write(code)
print("UserProfileModal.tsx written successfully")
